...
public:
  DSPL(byte RS, byte E, byte DB4, byte DB5, byte DB6, byte DB7) :
...
void DSPL::init(void) {
  ...
  //full_second_line = true;
    ...
    LiquidCrystal::print("SOLDERING");
    LiquidCrystal::setCursor(0, 1);
   LiquidCrystal::print(" STATION");
    delay(500);
    LiquidCrystal::clear();
    LiquidCrystal::print("BY: MANOJ");
    LiquidCrystal::setCursor(0, 1);
    LiquidCrystal::print("    GUPTA");
    delay(1000);
...
void DSPL::msgCelsius(void) {
  LiquidCrystal::setCursor(0, 1);
  LiquidCrystal::print(F("CELCIUS      "));
}

void DSPL::msgFarneheit(void) {
  LiquidCrystal::setCursor(0, 1);
  LiquidCrystal::print(F("FARENHEIGHT  "));
}
...
void DSPL::setupMode(byte mode, byte p) {
  ...
  case 1:
    ...
    if (p)
      LiquidCrystal::print("Cel.    ");
    else
      LiquidCrystal::print("Farh.   ");
    break;
  case 2:
    LiquidCrystal::print(F("CALIBRATION "));
    break;
  case 3:
    LiquidCrystal::print(F("TUNE"));
    break;
...
private:
  ...
  HISTORY h_power;
  HISTORY h_temp;
  const uint16_t temp_no_iron = 941; // Sensor reading when the iron disconnected
...
private:
  ...
  const uint16_t period = 500;             // The period to update the screen
};
...
private:
  ...
  const uint16_t period = 500;        // The period to update the screen (ms)
};
...
DSPL disp(LCD_RS_PIN, LCD_E_PIN, LCD_DB4_PIN, LCD_DB5_PIN, LCD_DB6_PIN,
    LCD_DB7_PIN);
